...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        N_i = np.array( [1] * len(self.clients) )
        C_i = np.array( [0] * len(self.clients) )
        sum_grad_sum = np.array( [0] * len(self.clients) )
        #sum_grads = np.zeros_like(self.clients[0].get_params()[0])  # Assuming the gradients shape will be the same as the model parameters
        ...
        for i in range(self.num_rounds):
            ...
            if self.clientsel_algo == 'submodular':
                ...
                indices, selected_clients, all_grad,N_i, sum_grad_sum, C_i = self.select_cl_submod(i, self.clients_per_round, N_i, sum_grad_sum, C_i,True)
            ...
            for idx, c in enumerate(active_clients.tolist()):  # simply drop the slow devices
                ...
                if self.clientsel_algo == 'submodular':
                    ...
                    if i == 0 or self.clients_per_round == 1:  # at the first iteration or when m=1, collect gradients from all clients
                        ...
                        self.sum_gradients = np.sum(self.all_grads, axis=0)
